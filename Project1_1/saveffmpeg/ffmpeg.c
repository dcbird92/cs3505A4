...
//     }
	...
	if(strstr(f->ctx->filename, ".png"))
	  {
	    printf("%s is a .png!\n", f->ctx->filename);
	    foundOne++;
	  }
	else if(strstr(f->ctx->filename, ".bmp"))
	  {
	    printf("%s is a .bmp!\n", f->ctx->filename);
	    foundOne++;
	  }
	else if(strstr(f->ctx->filename, ".gif"))
	  {
	    printf("%s is a .gif!\n", f->ctx->filename);
	    foundOne++;
	  }
    ...
    if(foundOne)
      {
	printf("*** CS 3505:  Executing in %s and %s\n",__FUNCTION__,__FILE__);
	printf("*** CS 3505:  Modified by Michael Sorger and Sean Hammond ***\n");
      }
